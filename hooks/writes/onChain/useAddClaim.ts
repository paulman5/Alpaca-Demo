...
export function useAddClaim() {
  ...
  const { isLoading: isConfirmingClaim, isSuccess: isClaimAdded } =
    useWaitForTransactionReceipt({ hash: addClaimHash });
  ...
  const addClaim = async ({
    ...
    try {
      ...
      const r = (signature.r.startsWith("0x") ? signature.r : `0x${signature.r}`) as `0x${string}`;
      const s = (signature.s.startsWith("0x") ? signature.s : `0x${signature.s}`) as `0x${string}`;
      const v = `0x${signature.v.toString(16).padStart(2, "0")}` as `0x${string}`;
      const sig = concatHex([r, s, v]) as `0x${string}`;
      const contractArgs = [topic, 1, issuerAddress as `0x${string}`, sig, claimDataHash, ""];
      writeContract({
  ...
  return { addClaim, isAddingClaim, isConfirmingClaim, isClaimAdded, error };
}
