...
const ordersABI = Array.isArray(ordersABIFile) ? ordersABIFile : (ordersABIFile as any).abi;
...
export function useOrdersContract() {
  ...
  function buyAsset(adfsFeedId: bigint, ticker: string, token: `0x${string}`, usdcAmount: bigint) {
  ...
  function sellAsset(adfsFeedId: bigint, ticker: string, token: `0x${string}`, tokenAmount: bigint) {
    return writeContract({
      address: ordersAddress as `0x${string}`,
      abi: ordersABI,
  ...
  return { buyAsset, sellAsset, data, isPending, isSuccess, error };
}
